...
void tf_callback(const tf::tfMessageConstPtr tf_msg) {
  for(int i=0; i < tf_msg->transforms.size(); i++){
    if(tf_msg->transforms[i].header.frame_id == "global" && tf_msg->transforms[i].child_frame_id == "cmu_rc1_map"){
    ...
    if(tf_msg->transforms[i].header.frame_id == "global" && tf_msg->transforms[i].child_frame_id == "cmu_rc2_map"){
    ...
    if(tf_msg->transforms[i].header.frame_id == "global" && tf_msg->transforms[i].child_frame_id == "cmu_rc3_map"){
...
void transform_rc1_to_darpa(const Eigen::Vector3d& pt, Eigen::Vector3d& transformed, ros::Time stamp) {
...
void transform_rc2_to_darpa(const Eigen::Vector3d& pt, Eigen::Vector3d& transformed) {
...
void transform_rc3_to_darpa(const Eigen::Vector3d& pt, Eigen::Vector3d& transformed) {
...
void callback(const nav_msgs::OdometryConstPtr rc1_odom, const nav_msgs::OdometryConstPtr rc2_odom, const nav_msgs::OdometryConstPtr rc3_odom) {
  if (!initialized) {
    return;
  }
  Eigen::Vector3d rc1_darpa_pt;
  transform_rc1_to_darpa(Eigen::Vector3d(rc1_odom->pose.pose.position.x, rc1_odom->pose.pose.position.y, rc1_odom->pose.pose.position.z), rc1_darpa_pt, rc1_odom->header.stamp);
  Eigen::Vector3d rc2_darpa_pt;
  transform_rc2_to_darpa(Eigen::Vector3d(rc2_odom->pose.pose.position.x, rc2_odom->pose.pose.position.y, rc2_odom->pose.pose.position.z), rc2_darpa_pt);
  Eigen::Vector3d rc3_darpa_pt;
  transform_rc3_to_darpa(Eigen::Vector3d(rc3_odom->pose.pose.position.x, rc3_odom->pose.pose.position.y, rc3_odom->pose.pose.position.z), rc3_darpa_pt);
...
int main( int argc, char** argv )
{
  ...
  message_filters::Subscriber<nav_msgs::Odometry> rc1_odom_sub(nh, "/cmu_rc1/aft_mapped_to_init_imu", 10);
  message_filters::Subscriber<nav_msgs::Odometry> rc2_odom_sub(nh, "/cmu_rc2/aft_mapped_to_init_imu", 10);
  message_filters::Subscriber<nav_msgs::Odometry> rc3_odom_sub(nh, "/cmu_rc3/aft_mapped_to_init_imu", 10);

  message_filters::Synchronizer<MySyncPolicy> sync(MySyncPolicy(10), rc1_odom_sub, rc2_odom_sub, rc3_odom_sub);
